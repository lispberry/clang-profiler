...
#include <iostream>
#include <vector>
#include <tree-sitter-cpp.h>  // Include tree-sitter-cpp header

// Function to parse a simple C++ source code snippet
void parse_code(const std::string& source_code) {
    // Create a new parser
    TSParser* parser = ts_parser_new();
    ts_parser_set_language(parser, tree_sitter_cpp()); // Set C++ language

    // Create a tree from the source code
    TSTree* tree = ts_parser_parse_string(
        parser, nullptr, source_code.c_str(), source_code.size()
    );

    if (!tree) {
        std::cerr << "Failed to parse C++ code." << std::endl;
    ...
    std::cout << "Parse tree: " << ts_node_string(root_node) << std::endl;
...
int main() {
    std::string cpp_code = "int main() { return 0; }";
    parse_code(cpp_code);
    return 0;
}
